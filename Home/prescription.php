<?php
    ...
    if(isset($_POST['upload-prescription'])){
       ...
       $image = $_POST['image'];
         ...
         $image_folder='pres/'.$image;
 
         
         
             $insert_product=mysqli_query($con,"INSERT INTO `prescription`(`name`,`age`,`number`,`email`,`address`,`image`)
             VALUES('$name','$age','$number','$email','$address','$image')") or die("query failed");
...
<div class="banner">
        <div class="detail">
            ...
            <p>Place your Information And 
                Upload a Clear Picture of Prescription.
            </p>
               <?php
        if(isset($message)){
...
<div>
        <h1 class="title">upload prescription Process</h1>
        ...
        <form method="post">
            ...
            <div class="input-field">
            <label>Recieved Address</lable>
                <input type="text" name="address" placeholder="enter pickup address">
        
            </div>
            <div class="input-field">
                    <label >Upload Your Valid Prescription</label>
                    <input type="file" name="image" id="image" value="image" accept="image/jpg, image/jpeg, image/webp, image/png" required>

                </div>
            
           
        <input type="submit" name="upload-prescription" class="btn" value="upload-prescription"  onclick="return confirm('are you sure to upload Prescription?')">
          
        </form></div>

            </div>
</div><footer class="full-footer">
